...
public class OlvidClient {
	private static final Logger LOGGER = LoggerFactory.getLogger(OlvidClient.class);
	...
	private OlvidClient(String daemonUrl, String clientKey, ChannelCredentials channelCredentials) {
		...
		if (clientKey != null) {
			this.clientKey = clientKey;
			LOGGER.debug("Using client key from builder argument");
			...
			LOGGER.debug("Using client key from OLVID_CLIENT_KEY environment variable");
		} else if (dotEnv.containsKey("OLVID_CLIENT_KEY")) {
			this.clientKey = dotEnv.getProperty("OLVID_CLIENT_KEY");
			LOGGER.debug("Using client key from .env file");
		} else {
			throw new IllegalStateException("No client key provided: set it via the builder, the OLVID_CLIENT_KEY environment variable, or a .env file");
		}
